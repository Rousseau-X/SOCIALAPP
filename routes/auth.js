...
router.post("/forgot-password", redirectIfAuth, async (req, res) => {
    try {
        ...
        if (!user.securityQuestion || !user.securityAnswer) {
            req.flash("error", "Aucune question de sécurité configurée pour ce compte. Contactez l'administrateur.")
            return res.redirect("/forgot-password")
        }
...
router.post("/register", redirectIfAuth, async (req, res) => {
    try {
        ...
        if (!securityQuestion || !securityAnswer || securityAnswer.trim().length < 2) {
            req.flash("error", "Veuillez choisir une question de sécurité et entrer une réponse.")
            return res.redirect("/register")
        }
